...
class MonthlyPriceRow(BaseModel):
    """One month of the wholesale-vs-retail wedge (native units; the UI converts)."""

    month: dt.date  # first of month, Europe/London
    system_gbp_mwh: float | None  # avg system (imbalance) price
    apx_gbp_mwh: float | None  # avg APX day-ahead reference (preferred wholesale line)
    agile_import_p_kwh: float | None  # avg Agile import rate, inc VAT


class FeeComponent(BaseModel):
    """One slice of what a capped unit rate pays for (approximate shares)."""

    name: str
    share_pct: float


class PeakFlex(BaseModel):
    """What the balancing market recently paid for peak flexibility."""

    window_days: int
    max_accepted_offer_gbp_mwh: float | None
    p90_accepted_offer_gbp_mwh: float | None
    median_accepted_offer_gbp_mwh: float | None
    avg_system_gbp_mwh: float | None  # same window, for contrast
    accepted_actions_7d: int


class Story(BaseModel):
    """Aggregates behind the narrative front page. Partial (constants only) rather
    than erroring while the warehouse is still warming up — the page must render."""

    window_from: dt.date | None
    window_to: dt.date | None
    monthly: list[MonthlyPriceRow]
    avg_agile_import_p_kwh: float | None
    price_cap_p_kwh: float
    price_cap_label: str
    fee_stack: list[FeeComponent]
    peak_flex: PeakFlex
